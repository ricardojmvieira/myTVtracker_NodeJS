                ...
                if (!found) {//se não existir este user name
                    if (Object.values(roles).indexOf(role) > -1) {//se no objeto dos roles existe algum role com o valor mandado por parametro
                        if (/^(?=.*[A-Z])(?=.*[a-z])(?=.*\d)[A-Za-z\d$@$!%*#?&-.]{8,}$/.test(rawPassword)) {
                            ...
                            db.collection('users').insertOne({ username, password, role, dataIv })//inserir o utilizador na db
                                .then(() => resolve())
    ...
    return new Promise((resolve, reject) => {
        db.collection('users')
            ...
            .then((user) => {//se encontrar é retornado
                if (user) {//se existir 
                    const password = cipher.decrypt(user.password, user.dataIv);//vamos decifrar a password da db com o vetor de inicializaçã que ja está na db
